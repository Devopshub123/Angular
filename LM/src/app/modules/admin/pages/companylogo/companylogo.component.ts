...
export class CompanylogoComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.getErrorMessages('LM35');
    this.getErrorMessages('LM36');
    this.getErrorMessages('LM61');
    this.getErrorMessages('LM62');
  ...
  removeImage() {
    ...
    this.LMS.getFilesMaster(info).subscribe((result) => {
      this.LMS.deleteFilesMaster(result.data[0].id).subscribe(data=>{

        if(data && data.status){
          let info =JSON.stringify(result.data[0])
          this.LM.removeImage(info).subscribe((data) => {})
  ...
  getErrorMessages(errorCode:any) {
    ...
    this.LM.getErrorMessages(errorCode,1,1).subscribe((result)=>{
      ...
      if(result.status && errorCode == 'LM34')
      {
        this.msgLM34 = result.data[0].errormessage
      }
      else if(result.status && errorCode == 'LM35')
      {
        this.msgLM35 = result.data[0].errormessage
      }
      else if(result.status && errorCode == 'LM36')
      {
        this.msgLM36 = result.data[0].errormessage
      }
      else if(result.status && errorCode == 'LM37')
      {
        this.msgLM37 = result.data[0].errormessage
      }
      else if(result.status && errorCode == 'LM61')
      {
        this.msgLM61 = result.data[0].errormessage
      }
      else if(result.status && errorCode == 'LM62')
      {
        this.msgLM62 = result.data[0].errormessage
      }
