...
export class AssignAssetsComponent implements OnInit {
  ...
  statusList: any = ['Assigned','Returned'];
  ...
  this.dataSource.paginator = this.paginator;
    this.assignAssetForm = this.formBuilder.group(
      ...
      this.assignAssetForm.controls.appliedDate.setValue(new Date());
  }
...
ngAfterViewInit() {
  ...
  Add(){
   // this.assignAssetForm.controls.companyName.setValue('');
    this.isAdd = true;
    this.isdata = false;
  }
  cancel(){
    this.router.navigateByUrl('/', { skipLocationChange: true }).then(() =>
      this.router.navigate(["/Asset/AssignAsset"]));
  }
}
