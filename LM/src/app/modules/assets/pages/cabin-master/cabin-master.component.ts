...
@Component({
  selector: 'app-cabin-master',
  templateUrl: './cabin-master.component.html',
  styleUrls: ['./cabin-master.component.scss']
})
export class CabinMasterComponent implements OnInit {
  cabinForm!: FormGroup;
  displayedColumns = ['sno','cid','location','subLocation','status','action'];
  ...
  locationsList: string[] = ['Hyderabad', 'Mumbai', 'Chennai','Bengalor'];
  isAdd:boolean=false;
  isdata: boolean = true;
  countryDetails: any = [];
  stateDetails: any = [];
    ...
    this.cabinForm = this.formBuilder.group(
      {
        ...
        ipAddress: [,],
        dataPort: [, Validators.required],
        voicePort: [, Validators.required],
...
applyFilter(event: Event) {
  ...
  Add(){
   // this.cabinForm.controls.companyName.setValue('');
    this.isAdd = true;
    this.isdata = false;
  }
  cancel(){
    this.router.navigateByUrl('/', { skipLocationChange: true }).then(() =>
      this.router.navigate(["/Asset/NewCabin"]));
  }
}
