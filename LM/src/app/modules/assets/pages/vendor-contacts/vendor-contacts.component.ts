...
export class VendorContactsComponent implements OnInit {
  ...
  isAdd:boolean=false;
  isdata: boolean = true;
  companyList: any = ['Sreeb','Sanela','Sanela Care','TCS'];
  ...
  this.dataSource.paginator = this.paginator;
    this.vendorContactForm = this.formBuilder.group(
      {
...
ngAfterViewInit() {
 ...
 applyFilter(event: Event) {
  ...
  Add(){
    //this.vendorContactForm.controls.companyName.setValue('');
    this.isAdd = true;
    this.isdata = false;
  }
  cancel(){
    this.router.navigateByUrl('/', { skipLocationChange: true }).then(() =>
      this.router.navigate(["/Asset/VendorContact"]));
  }
}
