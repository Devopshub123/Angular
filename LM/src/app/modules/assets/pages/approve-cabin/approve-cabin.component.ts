...
import { ApproveCabinDialogComponent } from '../approve-cabin-dialog/approve-cabin-dialog.component';
...
@Component({
  selector: 'app-approve-cabin',
  templateUrl: './approve-cabin.component.html',
  styleUrls: ['./approve-cabin.component.scss']
})
export class ApproveCabinComponent implements OnInit {
...
applyFilter(event: Event) {
  ...
  approveRequest() {
    this.isReject = false;
    this.dialog.open(ApproveCabinDialogComponent, {
      height: '70%',
      disableClose:true,
      data: this.isReject,
    });
  ...
  cancel(){
    this.router.navigateByUrl('/', { skipLocationChange: true }).then(() =>
      this.router.navigate(["/Asset/ApproveCabin"]));
  }
}
