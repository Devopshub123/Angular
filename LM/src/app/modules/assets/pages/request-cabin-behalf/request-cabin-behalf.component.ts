...
@Component({
  selector: 'app-request-cabin-behalf',
  templateUrl: './request-cabin-behalf.component.html',
  styleUrls: ['./request-cabin-behalf.component.scss']
})
export class RequestCabinBehalfComponent implements OnInit {
  requestCabinForm!: FormGroup;
  ...
  dataNotSave: any;
  ///////////
  reportManagerList: any = ['Manager1','Manager2'];
  ...
  this.dataSource.paginator = this.paginator;
    this.requestCabinForm = this.formBuilder.group(
      {
        appliedDate: [],
        reportManager: [],
        empName: [],
...
applyFilter(event: Event) {
  ...
  Add(){
   // this.requestCabinForm.controls.companyName.setValue('');
    this.isAdd = true;
    this.isdata = false;
  }
  cancel(){
    this.router.navigateByUrl('/', { skipLocationChange: true }).then(() =>
      this.router.navigate(["/Asset/RequestCabinBehalf"]));
  }
}
