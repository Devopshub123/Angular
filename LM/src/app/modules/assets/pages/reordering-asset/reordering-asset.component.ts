...
@Component({
  selector: 'app-reordering-asset',
  templateUrl: './reordering-asset.component.html',
  styleUrls: ['./reordering-asset.component.scss']
})
export class ReorderingAssetComponent implements OnInit {
  notificationsForm!: FormGroup;
  displayedColumns = ['sno','type','name','expDate','warDate','status','action'];
  ...
  dataNotSave: any;
  ///////////
  serviceTypeList: string[] = ['Software', 'Hardware'];
  statusValue:any=[{Value:'Active',name:'Active '},{Value:'Inactive',name:'Inactive'}];
  isAdd:boolean=false;
  isdata: boolean = true;
...
applyFilter(event: Event) {
  ...
  cancel(){
    this.router.navigateByUrl('/', { skipLocationChange: true }).then(() =>
      this.router.navigate(["/Asset/ReorderAsset"]));
  }
}
