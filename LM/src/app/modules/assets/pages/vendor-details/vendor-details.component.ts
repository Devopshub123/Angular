...
export class VendorDetailsComponent implements OnInit {
  ...
  dataNotSave: any;
  ///////////
  servicesList: string[] = ['Hardware', 'Software', 'Stationary'];
  isAdd:boolean=false;
  isdata: boolean = true;
  countryDetails: any = [];
  stateDetails: any = [];
  ...
  this.dataSource.paginator = this.paginator;
    this.vendorDetailsForm = this.formBuilder.group(
      {
        ...
        gstCtrl: [,],
        typeOfServiceCtrl: [, Validators.required],
    ...
    this.getCountry();
    this.vendorDetailsForm.get('countryCtrl')?.valueChanges.subscribe(selectedValue => {
      this.stateDetails = [];
      this.assetService.getStates(selectedValue).subscribe((data) => {
    ...
    this.vendorDetailsForm.get('stateCtrl')?.valueChanges.subscribe(selectedValue => {
      this.cityDetails = [];
      this.assetService.getCities(selectedValue).subscribe((data) => {
...
ngAfterViewInit() {
  ...
  getCountry() {
  this.countryDetails =[]
  this.assetService.getCountry('countrymaster', null, 1, 10, 'nandyala_hospitals').subscribe(result => {
...
applyFilter(event: Event) {
  ...
  Add(){
    this.vendorDetailsForm.controls.companyName.setValue('');
    this.isAdd = true;
    this.isdata = false;
  }
  cancel(){
    this.router.navigateByUrl('/', { skipLocationChange: true }).then(() =>
      this.router.navigate(["/Asset/VendorDetails"]));
  }
}
